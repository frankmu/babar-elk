...
module.controller('MlNewJobSingleStep1', (
  ...
  $scope.step2WithSearchUrl = (hit) => {
    return '#/jobs/new_job/simple/single_metric/create?savedSearchId=' + encodeURIComponent(hit.id);
  };
  $scope.makeUrl = (pattern) => {
    if (!pattern) return;
    return '#/jobs/new_job/simple/single_metric/create?index=' + encodeURIComponent(pattern);
  };
});
